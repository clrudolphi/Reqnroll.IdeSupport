...
**Last Updated**: 2026-05-28 14:36:51

## 📝 Plan Steps
-  **Retarget `Reqnroll.VisualStudio.VsxStubs.csproj` — remove `Reqnroll.VisualStudio.csproj` reference; add `Reqnroll.IdeSupport.Common`, `Reqnroll.IdeSupport.VisualStudio.VSSDKIntegration`, `Reqnroll.IdeSupport.LSP.Core`; fix all broken `using` / namespace references in each stub file to use the new `Reqnroll.IdeSupport.*` namespaces**
-  **Add lean LSP stubs to `VsxStubs` — create `LspStubs\StubGherkinTextSnapshot.cs` (implements `IGherkinTextSnapshot`), `StubLspIdeScope.cs`, `StubLspProjectScope.cs` with no VSSDK dependencies, for use by `LSP.Core.Tests` and `LSP.Server.Tests`**
-  **Retarget `Reqnroll.VisualStudio.Tests.csproj` — remove `Reqnroll.VisualStudio.csproj` reference; add `Reqnroll.IdeSupport.Common`, `Reqnroll.IdeSupport.VisualStudio.VSSDKIntegration`, `Reqnroll.IdeSupport.LSP.Core`, `Reqnroll.VisualStudio.VsxStubs`; fix namespaces in `FileUserIdStoreTests.cs`**
-  **Create `tests\Core\Reqnroll.IdeSupport.Common.Tests\Reqnroll.IdeSupport.Common.Tests.csproj` (net10, xunit) — move and rename `AnalyticsTransmitterTests`, `CSharpCodeGenerationConfigurationTests`, `ReqnrollConfigDeserializerTests`, `LoggingTests`, `ReqnrollPackageDetectorTests` into this project; update namespaces**
-  **Create `tests\LSP\Reqnroll.IdeSupport.LSP.Core.Tests\Reqnroll.IdeSupport.LSP.Core.Tests.csproj` (net10, xunit) — move and rename all `Discovery\ProjectBindingRegistry*`, `Discovery\BindingImporterTests`, `Discovery\ReprocessStepDefinitionFileTests` (+ `ApprovalTestData\` content), `Discovery\StubGherkinDocument`, `Editor\Services\DeveroomGherkinParserTests`, `Editor\TestFeatureFile`, `Editor\TestStepDefinition`; update namespaces**
-  **Write new `DeveroomTagParserTests.cs` in `LSP.Core.Tests` — comprehensive coverage of all `DeveroomTagTypes`: Feature/Rule/Scenario/ScenarioOutline blocks, step keywords, defined/undefined/ambiguous/binding-error steps with `StepDefinitionSampler`-level assertions, DataTable/DocString args, outline placeholders, hook references, parser error tags, multiline descriptions; uses `StubGherkinTextSnapshot` and a test-only `ProjectBindingRegistry` seeded via `BindingImporter`**
-  **Write new `GherkinDocumentContextCalculatorTests.cs` in `LSP.Core.Tests` — cursor-position-to-context mapping**
-  **Create `tests\LSP\Reqnroll.IdeSupport.LSP.Server.Tests\Reqnroll.IdeSupport.LSP.Server.Tests.csproj` (net10, xunit) — reshape `FeatureFileTaggerTests` / `TaggerSut` to use `GherkinDocumentTaggerService` + `LspTextSnapshot` (no VS `ITextBuffer`/`ITagSpan`)**
-  **Write new LSP Server test classes — `DocumentBufferServiceTests`, `LspWorkspaceScopeManagerTests`, `TextDocumentSyncHandlerTests`, `GherkinDocumentTaggerServiceTests`, `SemanticTokenServiceTests`, `SemanticTokensHandlerTests`, `WatchedFilesHandlerTests`, `WorkspaceFoldersHandlerTests` (see Part 5 for scenarios)**
-  **Create `tests\VisualStudio\Reqnroll.IdeSupport.VisualStudio.Wizards.Tests\Reqnroll.IdeSupport.VisualStudio.Wizards.Tests.csproj` (net481, xunit) referencing Wizards.Core + Wizards.UI + Wizards + VSSDKIntegration**
-  **Write wizard unit tests — `ReqnrollProjectTemplateWizardTests`, `ConfigFileTemplateWizardTests`, `FeatureFileTemplateWizardTests`, `AddNewReqnrollProjectViewModelTests`, `WelcomeDialogViewModelTests`, `UpgradeDialogViewModelTests`, `VsWizardContextTests` (see Part 4 for scenarios)**
-  **Retarget `Reqnroll.VisualStudio.Specs.csproj` — remove `Reqnroll.VisualStudio.csproj` and `DeploymentAssets.props`; add `Reqnroll.IdeSupport.Common`, `Reqnroll.IdeSupport.LSP.Core`, `Reqnroll.IdeSupport.LSP.Server`; remove the `NuGet.CommandLine` copy step (connector-dependent)**
-  **Reshape Specs step definitions for kept features — update `DeveroomSteps.cs` to use `GherkinDocumentTaggerService` + `LspTextSnapshot` + `StubLspProjectScope` in place of VS tagger/`StubWpfTextView`; mark deferred feature files with `@ignore` tags and a comment block explaining the missing LSP handler**
-  **Mark all deferred Specs feature files — add `@wip` or `@ignore` tag to the 9 command features, 5 discovery features, and 2 completion features with a comment referencing the to-do item (e.g., `# Deferred: requires textDocument/completion handler — see to dos.md §2c`)**
-  **Register all new test projects in `Reqnroll.IdeSupport.slnx` — add solution folders `tests/Core`, `tests/LSP`, update existing `tests/VisualStudio` folder entries**
