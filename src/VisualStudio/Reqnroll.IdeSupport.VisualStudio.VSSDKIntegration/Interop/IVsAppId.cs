...
public enum VSAPropID
{
    ...
    AltMSODLL =
        -8510, // BSTR - path/filename for alternate MSOx DLL (default - ask MSI), exactly as passed to LoadLibrary

    CreateProjShortcuts =
        -8511, // BOOL - default TRUE should shortcuts to solutions/projects be added to 'recent' folder?
    ...
    CmdLineOptStrFirst =
        -8522, // I4 - beginning of res id range of the Command Line Options string resource(s) in appid package

    // satellite dll. used instead of CmdLineOptDialog when output piped to console
    CmdLineOptStrLast =
        -8523, // I4 - end of res id range of the Command Line Options string resource(s) in appid package
    ...
    AllowsDroppedFilesOnMainWindow =
        -8529, // BOOL - default is TRUE.  Should the main window accept dropped files (i.e., WS_EX_ACCEPTFILES)?
    DisableAnswerWizardControl = -8530, // BOOL - default is TRUE.  Should the AnswerWizard menubar control be disabled?

    DisableAnsiCodePageCheck =
        -8531, // BOOL - default is FALSE.  Should the Ansi codepage check be used when loading UI libraries?

    DisableInstructionUnitStepping =
        -8532, // Set to TRUE to disable debugger's support for source-instruction stepping.

    UseVisualStudioDialogShortcuts =
        -8533, // BOOL - default is TRUE.  Should the VS shortcuts be used in the Open/Save/Browse dialogs?

    SKUEdition =
        -8534, // Either a VSASKUEdition or a string. VSASKUEdition if it is a standard version, or a BSTR if a custom version.
    Logo = -8535, // BSTR - logo for command line

    DDEApplication =
        -8536, // BSTR - application supported in DDE (expected in WM_DDE_INITIATE).  Required for DDE support.
    DDETopic = -8537, // BSTR - topic supported in DDE (expected in WM_DDE_INITIATE)  Required for DDE support.

    VSIPLicenseRequired =
        -8538, // BOOL - default is FALSE.  If TRUE, about box puts up stuff about VSIP license required

    DropFilesOnMainWindowHandler =
        -8539, // GUID - package GUID, which implements IVSDropFilesHandler to override default behaviour
    CmdLineError = -8540, // BSTR or I4 - error message for invalid cmd line to show before cmd line options help

    // (either a string or a resource id that is loaded from GuidAppIDPackage UILibrary)
    AllowCurrentUserSafeDomains =
        -8541, // BOOL - default is FALSE. Should security manager add safe domains from HKCU\<appid hive>\VsProtocol\SafeDomains
    TechSupportLink = -8542, // BSTR - should be link to tech support for this appid.

    HideMiscellaneousFilesByDefault =
        -8543, // BOOL - default is FALSE.  Should the Miscellaneous Files project be hidden by default?
    PredefinedAliasesString = -8544, // BSTR - predefined aliases for the appid

    ShowRuntimeInAboutBox =
        -8545, // BOOL - default is FALSE.  Should runtime (and runtime ver) show up at the top of about box?
    ...
    NewProjDlgSlnTreeNodeTitle =
        -8548, // BSTR or I4 localized replacement name for the 'Visual Studio Solutions' node in the 'Project Types' tree in

    // the New Project dialog. (either a string or a resource id that is loaded from GuidAppIDPackage UILibrary)
    DefaultProjectsLocation =
        -8549, // BSTR full path to the projects location (overrides the 'Visual Studio Projects' location)

    SolutionFileCreatorIdentifier =
        -8550, // BSTR string used as the second line in the solution file (used for determining SLN double-click behavior)

    HideSolutionExplorerToolbar =
        -8551, // BOOL - default is FALSE. Should the Solution Explorer tool window hide its toolbar?

    DefaultUserFilesFolderRoot =
        -8552, // BSTR name of folder at the end of the default my documents location, e.g. 'Visual Studio' in the default case: '%USERPROFILE%\My Documents\Visual Studio'

    UserFilesSubFolderName =
        -8553, // BSTR name of folder used for appid-specific subfolders under '%USERPROFILE%\My Documents\Visual Studio', e.g. 'Visual Basic Express' for '%USERPROFILE%\My Documents\Visual Studio\Settings\Visual Basic Express'

    NewProjDlgInstalledTemplatesHdr =
        -8554, // BSTR or I4 localized replacement name for the 'Visual Studio installed templates' header in the 'Templates' list

    // in the New Project dialog. (either a string or a resource id that is loaded from GuidAppIDPackage UILibrary)
    IncludeAddNewOnlineTemplateIcon =
        -8555, // BOOL - default is TRUE. Should the "Add New Online Template" icon be added to the New Project/Item dialogs?

    // Note: if this icon is not added then the "My Templates" group only shows up if other user templates are added.
    AddinsAllowed =
        -8556, // VARIANT_BOOL indicating wether Add-ins can be loaded or not. If not implemented, then VARIANT_TRUE is assumed.
    App64Icon = -8557, // I4 - HICON  for 64x64 icon

    UseAutoRecovery =
        -8558, // BOOL - default is TRUE. In order to turn off AutoRecovery, an AppID should implement this

    // propid and set its value to FALSE.
    DisableOutputWindow =
        -8559, // VARIANT_BOOL indicating whether shell should treat the output window as disabled. Returning VARIANT_TRUE means that
    ...
    StartPageTheme =
        -8561, // INT_PTR pointing to the memory containing the VSSTARTPAGETHEME struct. The memory should be allocated and de-allocated at the
    ...
    AutoRecoveryTookPlace =
        -8564, // VARIANT_BOOL indicating if an AutoRecovery took place. The default value is VARIANT_FALSE, and it is set to VARAINT_TRUE
    ...
    AboutBoxTheme =
        -8565, // INT_PTR pointing to the memory containing the VSABOUTBOXTHEME struct. The memory should be allocated and de-allocated at the

    // appid implementation level.
    SQMTitle = -8566, // BSTR Title for the SQM optin dialog.

    AutoSaveNewUnsavedFiles =
        -8567, // VARIANT_BOOL indicating whether the disaster recovery mechanism should save previously unsaved files.  The default is FALSE.
    ...
    SupportRestartManager =
        -8578, // VT_BOOL (default is TRUE). In order to turn off support for Restart Manager, an AppID should implement this
    ...
    CommonExtensionSearchPath =
        -8584, // SafeArray of BSTRs. APPID specific list of folders where to look for Common (shared by all users) VS extensions.
    ...
    LoadUserExtensions =
        -8586, // VT_BOOL. Tells PkgDef management and Extension Manager API whether to load User extensions.
    ...
    AllowLoadingAllPackages =
        -8588, // VT_BOOL. Each APPID specifies through this property if it allows loading ALL Visual Studio Packages
    ...
    VSAPROPID_SplashScreenTheme =
        -8591, // INT_PTR pointing to the memory containing the VSSPLASHSCREENTHEME struct. The memory should be allocated and
    ...
    VSAPROPID_ApplicationRootFolder =
        -8593, // BSTR Full path of root location of installation (e.g. drive>:\Program Files\Microsoft Visual Studio <version>\)

    VSAPROPID_ApplicationExtensionsFolder =
        -8594, // BSTR Full path of folder for installing per-machine Extensions (e.g. Example: C:\Program Files\Microsoft Visual Studio <VS version>\Common7\IDE\Extensions)

    VSAPROPID_GenericTheme =
        -8595, // INT_PTR pointing to the memory containing the VSGENERICTHEME struct. The memory should be allocated and

    // de-allocated at the appid implementation level.
    VSAPROPID_ActivityLogPath = -8596, // VT_BSTR, Read-Only. Path to ActivityLog file.

    VSAPROPID_ReleaseVersion =
        -8597, // VT_BSTR, Read-Only. The build version of the release and the branch/machine/user information used to build it (e.g. "10.0.30319.01 RTMRel" or "10.0.30128.1 BRANCHNAME(COMPUTERNAME-USERNAME)"). This is the same as the release string shown in Help/About.

    VSAPROPID_EnableSamples =
        -8598, // VT_BOOL. Specifies whether samples are enabled. Defaults to false if not specified for Isolated Shell appids.

    VSAPROPID_EnableMicrosoftGalleries =
        -8599, // VT_BOOL. Specifies whether Microsoft-owned extension galleries are enabled. Defaults to false if not specified for Isolated Shell appids.

    VSAPROPID_EnablePrivateGalleries =
        -8600, // VT_BOOL. Specifies whether private extension galleries are enabled. Defaults to false if not specified for Isolated Shell appids.

    VSAPROPID_AppVectorIcon =
        -8601, // VT_BSTR. Gets a vector path for an icon. This vector path must conform to the path markup syntax used by System.Windows.Media.Geometry.

    VSAPROPID_AppBrandName =
        -8602, // VT_BSTR. The localized full brand name of the application, including SKU information. E.g. "Microsoft Visual Studio Professional 2012 RC" or "Microsoft Visual Studio Express 2012 RC for Windows 8"

    VSAPROPID_AppShortBrandName =
        -8603, // VT_BSTR. A short version of VSAPROPID_AppBrandName, less than 32 chars. E.g. "VS Pro 2012 RC" or "VS Express 2012 RC for Win8"

    VSAPROPID_SKUInfo =
        -8604, // VT_BSTR. A localized text describing the current SKU (name, year, release type, etc). E.g. "Ultimate 2012 RC" or "Express 2012 RC for Web"

    VSAPROPID_GuidDefaultColorTheme =
        -8605, // GUID representing the color theme that should be used by default for the appid. If unimplemented by the appid, or if the theme does not exist when the appid is launched, the default light theme is chosen.
    ...
    VSAPROPID_IsSubscriptionAware =
        -8610, // VT_BOOL. Specifies whether the application supports subscription license from VS Online

    VSAPROPID_SubscriptionLicenseId =
        -8611, // GUID unique LicenseID that application specifies under $RootFolder$\Licenses for coordinating its VS Online subscription tokens.

    VSAPROPID_SubscriptionRightsName =
        -8612, // VT_BSTR. Unique Name that identifies this application with the VS Online Licensing Service.

    VSAPROPID_SupportsConnectedUser =
        -8613, // VT_BOOL. Specifies whether the application supports Connected User UI (e.g. Connected User sign-in, ID Card, roaming settings, first launch sign-in invitation, etc.)

    VSAPROPID_SettingsRegistryRoots =
        -8614, // SafeArray of BSTRs, in order from earliest to latest, including current version, of registry roots checked during settings migration

    VSAPROPID_EnableOfflineHelpNotification =
        -8615, // VT_BOOL. Specifies whether the help notification should be published to the notification hub on first launch

    VSAPROPID_DefaultProfile =
        -8616, // VT_BSTR (optional). Specifies the default profile for the appid (e.g. "General")

    VSAPROPID_ThemeThumbnailProvider =
        -8617, // VT_UNKNOWN (optional). Specifies an IUnknown from which the IVsThemeThumbnailProvider interface for the appid can be queried.

    VSAPROPID_CommunityEdition =
        -8618, // VT_BOOL. Specifies whether VS is community edition. Only applicable to VS Professional.
    ...
    VSAPROPID_AppQueryLoadServiceObject =
        -8621, // VT_UNKNOWN. IUnknown the free threaded app query load service object.

    VSAPROPID_IsVSTelemetryEnabled =
        -8622, // VT_BOOL. Specifies whether the VS Telemetry API is enabled in the SKU or not.
    VSAPROPID_WorkingFodersRootExt = -8623, // BSTR - solution working folders extension(default - "");

    VSAPROPID_UnlocalizedReleaseString_Short =
        -8624, // BSTR what this release is branded as, e.g. November CTP, Beta 2, etc. (unlocalized)
    ...
    VSAPROPID_IsolationInstallationUserDataFilePath =
        -8632, // VT_BSTR. The AppId's isolation installation userdata file path.
    ...
    VSAPROPID_LegacyCompatDirectory =
        -8635, // VT_BSTR. The root legacy compat directory that MSIs that are not isolation aware may install things to

    VSAPROPID_CommonExtensionExclusionList =
        -8636, // SafeArray of BSTRs. A list of directories to exclude from extension processing (pkgdef, MEF, etc..)
    ...
    VSAPROPID_ChannelManifestId =
        -8639, // VT_BSTR. The AppId's installation channel manifest unique ID, for example VisualStudio.15.Release/public.d15rel/15.0.26020.0

    VSAPROPID_InstallationNickname =
        -8640, // VT_BSTR. The AppId's installation nickname to disambiguate between SxS installations.
