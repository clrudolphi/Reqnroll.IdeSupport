...
Three features still have **no entry in `plugin.xml` at all** because they aren't implemented yet: Comment Toggle, Rename Step, and the document outline/structure view — see reqnroll/Reqnroll.IdeSupport#159–160, #163.
...
| `ReqnrollRequestSender` | Sends the custom `reqnroll/findStepUsages`/`reqnroll/findUnusedStepDefinitions`/`reqnroll/goToHooks` requests, plus the *standard* `textDocument/codeLens`/`inlayHint`/`onTypeFormatting`/`foldingRange` requests that Rider's generic client has no rendering-side consumer for |
...
| F13 · Comment/Uncomment | Glue — `workspace/executeCommand` (`reqnroll.toggleComment`) bound to Ctrl+/, applied via the client's native `workspace/applyEdit` handling | Glue — same standard `workspace/executeCommand` route, bound via a VSSDK command-filter redirect | ❌ Not implemented — no `Commenter`/action registered at all (#159) | No |
...
> **This table supersedes the per-feature "IDE support matrix" tables in the [Feature Designs](LSP-IDE-Support-Feature-Designs.md) doc wherever they disagree.** Those tables were written at design time (some before the Rider plugin existed) and predicted several Rider cells as `✅ Generic` that turned out, once built, to need `Glue` (F1, F10, F18, F23) or aren't implemented at all yet (F9, F13, F16) — this table reflects the current as-built/tracked reality; the per-feature tables have not all been individually corrected yet (F10 and F17 have been).
