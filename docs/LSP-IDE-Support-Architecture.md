...
| `postStartupActivity` | `ReqnrollCommentToggleInstaller` | Installs the Comment/Uncomment toggle handler (F13) |
...
Three features still have **no entry in `plugin.xml` at all** because they aren't implemented yet: Go to Hooks, Rename Step, and the document outline/structure view (folding is also unimplemented) — see reqnroll/Reqnroll.IdeSupport#158, #160, #162–163.
...
| `ReqnrollCommentToggleHandler` / `ReqnrollCommentToggleInstaller` | F13 — decorates the shared, application-global `CommentByLineComment` `EditorActionHandler` (there is no `LanguageCommenter` for `.feature`, and no declarative extension point for "wrap an existing editor action handler") so it also dispatches `reqnroll.toggleComment` on `.feature` files |
...
| F13 · Comment/Uncomment | Glue — `workspace/executeCommand` (`reqnroll.toggleComment`) bound to Ctrl+/, applied via the client's native `workspace/applyEdit` handling | Glue — same standard `workspace/executeCommand` route, bound via a VSSDK command-filter redirect | Glue — `ReqnrollCommentToggleHandler` decorates the built-in `CommentByLineComment` handler and sends the same `workspace/executeCommand` directly; the resulting `workspace/applyEdit` is applied natively by Rider's platform `Lsp4jClient`, no consumer glue needed for that half (#159) | No |
...
| F16 · Step Rename Refactoring | Custom — `renameDisambiguation.ts` (`reqnroll/renameTargets` + `reqnroll/selectRenameTarget`) atop standard `textDocument/rename` | Custom — same messages + `RenameStep/*` | ❌ Not implemented (#160) | **Yes** — `RenamePostApplyCoordinator` branches on `IsVisualStudio` for post-apply handling |
| F17 · Go to Hooks | Custom — `reqnroll/goToHooks` + `QuickPick` | Custom — same message + `NavigationPickerDialog` | ❌ Not implemented (#158) | No |
...
> **This table supersedes the per-feature "IDE support matrix" tables in the [Feature Designs](LSP-IDE-Support-Feature-Designs.md) doc wherever they disagree.** Those tables were written at design time (some before the Rider plugin existed) and predicted several Rider cells as `✅ Generic` that turned out, once built, to need `Glue` (F1, F13, F18, F23) or aren't implemented at all yet (F9, F10, F16, F17) — this table reflects the current as-built/tracked reality; the per-feature tables have not all been individually corrected yet (F13 has been).
...
text-sync), not just the original semanticTokens/completion/definition/diagnostics-push set.
